...
app.post('/v1/chat/completions', async (req, res) => {
  ...
  try {
    ...
    console.log(`[${new Date().toISOString()}] Request received - Model: ${model}, Stream: ${stream}, Messages: ${messages?.length || 0}`);
    ...
    console.log(`[${new Date().toISOString()}] Selected NIM model: ${nimModel}`);
    ...
    if (stream) {
        ...
        lines.forEach(line => {
          if (line.startsWith('data: ')) {
            if (line.includes('[DONE]')) {
              console.log(`[${new Date().toISOString()}] Stream completed - ${chunkCount} chunks`);
              res.write(line + '\n');
              return;
            }
            ...
            try {
              ...
              console.error(`[${new Date().toISOString()}] Error parsing stream chunk:`, e.message);
              res.write(line + '\n');
            }
          }
        });
      ...
      response.data.on('end', () => {
        console.log(`[${new Date().toISOString()}] Stream ended - Total time: ${Date.now() - startTime}ms`);
        streamActive = false;
        res.end();
      });
      ...
      const pingInterval = setInterval(() => {        if (streamActive && Date.now() - lastChunkTime > 15000) {
          res.write(': ping\n\n');
        }
      }, 15000);
      ...
      console.log(`[${new Date().toISOString()}] Non-streaming response received`);
      ...
      const openaiResponse = {
        ...
        choices: response.data.choices.map(choice => {
          ...
          if (SHOW_REASONING && choice.message?.reasoning_content) {
            fullContent = 'app.get('/health', (req, res) => {
  res.json({ 
    status: 'ok', 
    service: 'OpenAI to NVIDIA NIM Proxy', 
    reasoning_display: SHOW_REASONING,
